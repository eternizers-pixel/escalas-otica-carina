...
window.Engine = (function () {
  ...
  function dayoffQueue(employees, rules, history={}, existing=[]){
    ...
    rows.forEach((x,i)=>{ x.position=i+1; const w=[];
      ...
      else ws.push(`sem saldo p/ folga · você tem ${fH(x.proj)}, mas para folgar precisa de ${fH(minBank+folgaCost)}: o mínimo de ${fH(minBank)} tem que sobrar no banco depois da folga de ${fH(folgaCost)}`);
      x.whyShort = ws.join(' · ');
    });
    return rows;
  }
