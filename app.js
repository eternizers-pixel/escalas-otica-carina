...
async function reconcileBank(){
  ...
  const [curBal,prevBal,scheds,vacs]=await Promise.all([
...
ROUTES.eventos=async function(){
  ...
  const eventCard=(ev)=>{
    ...
    // LOG DE DECISÃO — horas do evento por pessoa (cada turno=4h; conta todos os turnos do evento)
    const carga={}; ev.items.forEach(i=>{ const id=i.employee_id; carga[id]=carga[id]||{n:0,e:0,t:0}; if(i.shift==='noite'){ carga[id].n++; } else { carga[id].e++; } carga[id].t+=4; });
